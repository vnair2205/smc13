// admin-panel/src/pages/ChurnReportPage.js
import React, { useState, useEffect, useCallback } from 'react';
import styled from 'styled-components';
import { FaFileCsv, FaSearch } from 'react-icons/fa';
import { getChurnReport } from '../services/reportService';
...
const ChurnReportPage = () => {
    ...
    const fetchReport = useCallback(async () => {
        setLoading(true);
        try {
            const response = await getChurnReport({ page: currentPage, limit: 100, search: searchTerm });
            setReportData(response);
        } catch (error) {
            console.error('Failed to fetch churn report:', error);
        } finally {
            setLoading(false);
        }
    ...
    const downloadCSV = () => {
        ...
        const headers = ['First Name', 'Last Name', 'Email', 'Phone', 'Last Active Plan', 'Last Subscription Date'];
        const rows = reportData.churnedCustomers.map(user => [
            user.firstName,
            user.lastName,
            user.email,
            user.phoneNumber || 'N/A',
            user.lastActivePlan,
            new Date(user.lastSubscriptionDate).toLocaleDateString()
        ]);
    ...
    const columns = [
        ...
        { title: 'Last Active Plan', key: 'lastActivePlan' },
        { title: 'Subscription Date', render: (row) => new Date(row.lastSubscriptionDate).toLocaleDateString() },
    ];
    ...
    return (
        <PageContainer>
            <Title>Churned Customers Report</Title>
            <StatsContainer>
                <StatCard title="Total Churned Customers" value={reportData.stats.totalChurned} type="inactive" />
            </StatsContainer>
            <ControlsContainer>
                <SearchContainer>
                    <SearchIcon />
            ...
            <DataTable columns={columns} data={reportData.churnedCustomers} />
