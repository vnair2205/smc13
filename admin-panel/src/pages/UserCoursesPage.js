...
const UserCoursesPage = () => {
    ...
    const handleViewCourse = async (courseId) => {
        try {
            ...
            // Find the first subtopic and first lesson
            if (course && course.index?.subtopics?.length > 0 && course.index.subtopics[0].lessons?.length > 0) {
                const firstSubtopicId = course.index.subtopics[0]._id;
                const firstLessonId = course.index.subtopics[0].lessons[0]._id;
                
                // Navigate to the full, correct URL
                navigate(`/admin/course-player/${courseId}/lesson/${firstSubtopicId}/${firstLessonId}`);
            } else {
                // If the course has no lessons, maybe navigate to an overview page or show an alert
                alert('This course does not have any lessons to view.');
                // As a fallback, you could navigate to a course overview if you have one
