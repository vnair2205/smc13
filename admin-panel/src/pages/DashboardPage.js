...
// --- STYLED COMPONENTS ---
const DashboardContainer = styled.div` padding: 2rem; background-color: #161625; `;
const Header = styled.div` margin-bottom: 2rem; `;
const Title = styled.h1` color: #fff; font-size: 2rem; `;
const Subtitle = styled.p` color: #a0a0b0; `;
const StatsGrid = styled.div` display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1.5rem; margin-bottom: 2.5rem; `;
const StatCard = styled.div` background: #1e1e32; border-radius: 10px; padding: 1.5rem; display: flex; align-items: center; gap: 1rem; border-left: 4px solid ${props => props.color || '#5e72e4'}; `;
const StatIcon = styled.div` font-size: 2.5rem; color: ${props => props.color || '#5e72e4'}; `;
const StatInfo = styled.div` h3 { margin: 0; font-size: 2rem; color: #fff; } p { margin: 0; color: #a0a0b0; font-size: 0.9rem; } `;
const ChartGrid = styled.div` display: grid; grid-template-columns: 1fr; gap: 2rem; @media (min-width: 1200px) { grid-template-columns: 2fr 1fr; } `;
const ChartContainer = styled.div` background: #1e1e32; border-radius: 10px; padding: 2rem; height: 450px; `;
const ChartTitle = styled.h2` color: #fff; margin-top: 0; margin-bottom: 2rem; font-size: 1.25rem; `;
const FilterContainer = styled.div` display: flex; justify-content: space-between; align-items: center; margin-bottom: 2rem; `;
const StyledSelect = styled.select` padding: 0.5rem 1rem; border-radius: 8px; border: 1px solid #33333e; background-color: #161625; color: white; `;
const ErrorMessage = styled.p` color: #ff5b5b; text-align: center; font-size: 1.2rem; `;

const COLORS = ['#03D9C5', '#5E72E4', '#FF5B5B', '#FFC107', '#6f42c1', '#2dce89'];
...
const DashboardPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setError("Could not load dashboard data. Please try again later."); // Set error message
            } finally {
                setLoading(false);
            }
        };
    ...
    const { years, revenueData } = useMemo(() => {
        ...
        return { years: ['All', ...Array.from(yearSet).sort().reverse()], revenueData: revenueDataFormatted };
    }, [data]);
    ...
    if (error) {
        return <DashboardContainer><ErrorMessage>{error}</ErrorMessage></DashboardContainer>;
    }
    if (!data) {
        return <DashboardContainer><ErrorMessage>No data available to display.</ErrorMessage></DashboardContainer>;
    }
    ...
    return (
        <DashboardContainer>
            {/* ... (The rest of the JSX remains the same as the previous correct version) ... */}
            <Header>
                <Title>Admin Dashboard</Title>
                <Subtitle>An overview of your platform's key metrics.</Subtitle>
            </Header>
            ...
            <StatsGrid>
                ...
                <StatCard color="#ffc107"><StatIcon color="#ffc107"><FaFolderOpen /></StatIcon><StatInfo><h3>{stats.openTickets}</h3><p>Open Tickets</p></StatInfo></StatCard>
                 ...
                 <ChartContainer>
                    ...
                    <ResponsiveContainer width="100%" height="85%">
                        <BarChart data={charts.revenueByPlan} layout="vertical">
                            ...
                            <YAxis type="category" dataKey="name" stroke="#a0a0b0" width={120} tick={{ fill: '#a0a0b0' }} />
                            <Tooltip contentStyle={{ backgroundColor: '#161625', border: '1px solid #33333e' }} />
                            <Legend />
