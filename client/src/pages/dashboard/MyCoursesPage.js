...
const SearchFilterContainer = styled.div`
    ...
    /* --- FIX 1: Ensure container and its children stack correctly on mobile --- */
...
const FilterControl = styled.div`
    flex-grow: 1; /* Allows items to grow on desktop if space is available */

    @media (max-width: 768px) {
        width: 100%; /* Each control takes the full width on mobile */
    }
`;
...
const InputGroup = styled.div`
    ...
    display: flex;
    align-items: center;
    width: 100%; /* Make the group take full width of its parent */
`;
...
const SearchInput = styled.input`
    ...
    color: white;
    font-size: 1rem;
    width: 100%; /* --- FIX 3: Ensure input fills its container --- */
    &:focus {
        outline: none;
...
const Select = styled.select`
    ...
    font-size: 1rem;
    cursor: pointer;
    width: 100%; /* --- FIX 3: Ensure select fills its container --- */
    &:focus {
        outline: none;
        border-color: ${({ theme }) => theme.colors.primary};
    }
