...
// --- FIX 1: Update the container to be collapsible ---
const SidebarContainer = styled.div`
  ...
  /* On mobile, this transform property hides/shows the sidebar */
  @media (max-width: 768px) {
    transform: ${({ $isRTL, $isOpen }) => {
        if ($isRTL) {
            return $isOpen ? 'translateX(0)' : `translateX(${expandedWidth})`;
        }
        return $isOpen ? 'translateX(0)' : `translateX(-${expandedWidth})`;
    }};
  }
...
const CourseSidebar = ({ isRTL, course, isOpen, setIsOpen }) => {
...
const startQuiz = () => {
    ...
    return (
        <>
         ...
         <SidebarContainer $isRTL={isRTL} $isOpen={isOpen}>
                <SidebarHeader>
