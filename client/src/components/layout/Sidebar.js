...
const SidebarContainer = styled.div`
  width: ${({ $isCollapsed }) => ($isCollapsed ? '88px' : '300px')};
  ...
  position: fixed;
  top: 0;
  left: ${({ dir }) => (dir === 'rtl' ? 'auto' : '0')};
  right: ${({ dir }) => (dir === 'rtl' ? '0' : 'auto')};
...
const Sidebar = ({ isCollapsed, setIsCollapsed, className }) => {
    ...
    const confirmLogout = async () => {
        ...
        try {
            ...
            console.error("Logout failed, clearing client session.", error);
        } finally {
    ...
    const menuItems = [
        ...
        { type: 'separator' },
         { 
            nameKey: 'sidebar.help_support', 
            icon: <FiHelpCircle />,
            subItems: [
                { path: "/knowledge-base", nameKey: "sidebar.knowledge_base" },
                { path: "/support-tickets", nameKey: "sidebar.support_tickets" },
            ]
        },
    ...
    return (
        <>
            ...
            <SidebarContainer className={className} $isCollapsed={isCollapsed} dir={document.body.dir}>
