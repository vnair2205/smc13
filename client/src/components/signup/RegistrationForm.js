...
const RegistrationForm = ({ plan, onBack, onSuccess }) => {
    ...
    const validateForm = () => {
      ...
      const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Instead of navigating, call the onSuccess prop
            // and pass the user's data back to the parent component.
            if (onSuccess) {
                onSuccess({ email: res.data.email, phone: res.data.phone });
            }
        ...
        <FormContainer>
            ...
            <Form onSubmit={handleSubmit}>
                ...
                {errors.agreed && <CheckboxErrorText>{errors.agreed}</CheckboxErrorText>}
                  <SignUpButton type="submit">{t('create_account_button')}</SignUpButton>
            </Form>
            <SignInText>
