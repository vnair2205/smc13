...
CourseSchema.index({ isPreGenerated: 1, status: 1 }); // For filtering
CourseSchema.index({ topic: 'text' }); // For text search on course topics
CourseSchema.index({ createdAt: -1 }); // For sorting by creation date
CourseSchema.index({ user: 1 }); // For queries related to a specific user
