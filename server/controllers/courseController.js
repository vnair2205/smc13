...
// **FIX:** A much more robust function to find the most relevant video for each lesson.
const findBestVideo = (videos, { lessonTitle, subtopicTitle, courseTopic }) => {
    ...
    const scoredVideos = videos
        .map(video => {
            ...
            // Penalize irrelevant video types
            if (title.includes('#shorts') || description.includes('#shorts') || title.startsWith('live:') || title.includes('livestream')) {
                return { video, score: -100 };
            }

            let score = 0;

            // High reward for including the exact lesson title
            if (title.includes(lessonTitle.toLowerCase())) {
                score += 20;
            }

            // Reward for including keywords from the lesson title
            ...
            // Moderate reward for including the subtopic title
            if (title.includes(subtopicTitle.toLowerCase())) {
                score += 5;
            }

            // Small reward for the main course topic
            if (title.includes(courseTopic.toLowerCase())) {
                score += 1;
            }
            
            // Penalize generic titles that are likely to be repeated
            if (title.includes('introduction') || title.includes('course overview') || title.includes('tutorial for beginners')) {
                score -= 5;
            }
    ...
    if (scoredVideos.length === 0) {
        return null; // No valid videos found at all
    }

    // Sort by the highest score
    scoredVideos.sort((a, b) => b.score - a.score);

    // Return the best video, but only if it has a positive score
    if (scoredVideos[0].score > 0) {
        return scoredVideos[0].video;
    }

    // If no video has a positive score, it's better to return nothing
    // than to return a completely irrelevant video.
    return null;
};
...
exports.generateLessonContent = async (req, res) => {
    ...
    try {
        ...
        if (shouldGenerateNewContent) {
            console.log('[LOGIC] Generating new content for lesson.');
            ...
            try {
                ...
                if (bestVideo) {
                    ...
                    console.warn('[YouTube] No suitable video found after filtering. Lesson will have no video.');
                }
            } catch (youtubeError) {
            ...
            lesson.videoHistory = videoUrl ? [{ videoUrl: videoUrl, videoChannelId: videoChannelId, videoChannelTitle: videoChannelTitle }] : [];
        ...
        await course.save();
        console.log('[DB] Course saved with updated lesson content.');
...
exports.changeLessonVideo = async (req, res) => {
    ...
    try {
        ...
        const existingVideoIds = new Set(lesson.videoHistory.map(v => v.videoUrl.split('/embed/')[1]));
        const newVideos = youtubeResponse.data.items.filter(v => v.id.videoId && !existingVideoIds.has(v.id.videoId));
    ...
    const options = {
     ...
     if (search) {
         query.$or = [
             ...
             { englishTopic: { $regex: search, $options: 'i' } }
         ];
    }

 if (certified === 'true') {
        const completedCoursesQuery = { user: userId, status: 'Completed' };
        const completedCoursesOptions = {
             page: parseInt(page, 10),
             limit: 100, 
             sort: { createdAt: -1 }
        };
        ...
        try {
            const allCompletedCourses = await Course.paginate(completedCoursesQuery, completedCoursesOptions);

            const certifiedCourses = allCompletedCourses.docs.filter(course => {
                const hasScore = typeof course.score === 'number';
            ...
            res.json({
                ...allCompletedCourses,
                docs: certifiedCourses,
                totalDocs: certifiedCourses.length
            });
            return;

        } catch (error) {
            console.error("Error fetching or filtering certified courses:", error.message);
            return res.status(500).json({ msgKey: "errors.generic" });
        }
