...
exports.getUsersCourses = async (req, res) => {
  ...
  if (certified === 'true') {
        // First, simply find all courses for the user that are marked as 'Completed'.
        // We set a higher limit to ensure we get all potential certificates.
        const completedCoursesQuery = { user: userId, status: 'Completed' };
        const completedCoursesOptions = {
             page: parseInt(page, 10),
             limit: 100, // Fetch up to 100 completed courses to filter
             sort: { createdAt: -1 }
        };
        ...
        try {
            ...
            // Now, filter these results safely in our code.
            const certifiedCourses = allCompletedCourses.docs.filter(course => {
                // A course is certified if the score is a number, quiz is an array with questions,
                // and the score is 60% or higher.
                const hasScore = typeof course.score === 'number';
                const hasQuiz = Array.isArray(course.quiz) && course.quiz.length > 0;

                if (!hasScore || !hasQuiz) {
                    return false; // Not certifiable if score or quiz is missing.
                }
            ...
            // Return the filtered results in the same format as the original paginate response.
            res.json({
                ...allCompletedCourses,
...
exports.getAdminLessonContent = async (req, res) => {
    try {
        // Find the course and populate only the user details
        const course = await Course.findById(req.params.courseId)
            .populate('user', 'firstName lastName'); // ✅ CORRECTED

        if (!course) {
            // If no course is found, send a 404 error
            return res.status(404).json({ msg: 'Course not found' });
        }

        // If the course is found, send it back as JSON
        res.json(course);
    } catch (err) {
