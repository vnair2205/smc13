...
exports.getUserStats = async (req, res) => {
    try {
        // 1. Get total number of users. This is correct.
        const total = await User.countDocuments();

        // --- FIX STARTS HERE ---
        // 2. Get the user IDs from all subscriptions marked as 'active'.
        const activeSubscriptionUserIds = (await Subscription.find({ status: 'active' }).select('user')).map(sub => sub.user);

        // 3. Count how many USERS exist with those IDs. This is the crucial step.
        // It filters out any subscriptions that might point to deleted users.
        const active = await User.countDocuments({ _id: { $in: activeSubscriptionUserIds } });
        // --- FIX ENDS HERE ---

        // 4. Inactive users are now calculated correctly without the risk of a negative number.
...
exports.getUserDetails = async (req, res) => {
    try {
        ...
        // Also fetch the user's entire subscription history
        const subscriptionHistory = await Subscription.find({ user: req.params.id })
            .populate('plan', 'name')
            .sort({ startDate: -1 });

        res.json({ ...user.toObject(), subscriptionHistory });
...
exports.changeUserPlan = async (req, res) => {
    ...
    try {
        ...
        // Expire the old subscription if it exists
