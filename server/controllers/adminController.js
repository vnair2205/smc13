...
exports.getUserStats = async (req, res) => {
    try {
        ...
        const activeSubscriptions = await Subscription.find({ status: 'active' }).distinct('user');
...
exports.getUserDetails = async (req, res) => {
    try {
        ...
        // 👇 FIX: Ensure the 'plan' field within each subscription history item is fully populated.
        const subscriptionHistory = await Subscription.find({ user: req.params.id })
            .populate({
                path: 'plan',
                model: 'SubscriptionPlan' // Explicitly populate the plan details
            })
            .sort({ startDate: -1 });
