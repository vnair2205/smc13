...
exports.createOrder = async (req, res) => {
    try {
        ...
        // 1. Use the planId from the request body if it exists (for plan upgrades).
        // 2. Otherwise, use the plan the user selected during signup.
        const planIdToUse = req.body.planId || user.selectedPlan;

        if (!planIdToUse) {
            return res.status(400).json({ msg: 'No subscription plan ID was found.' });
        }
